...
async fn init() -> (UserContext, TaskID, TaskService<MockTaskRepository>) {
    ...
    let task = task_service
        ...
        .await
        .unwrap();
    task_service.complete(task.id, user_context).await.unwrap();
...
mod success {
    ...
    async fn success() {
        ...
        let res = task_service.reopen(task_id, user_context).await;
        assert!(res.is_ok());
    }
...
mod existence {
    ...
    async fn not_exists() {
        ...
        let res = task_service.reopen(TaskID::new_v4(), user_context).await;
        assert!(res.is_err());
        if let Err(err) = res {
            assert!(matches!(err, Error::NotFound(Resource::Task)))
        }
...
mod error {
    ...
    async fn repo_programming_error() {
        let user_service = TaskService::init(MockTaskRepository::new_programming_error());
        let user_context = UserContext {
            id: UserID::new_v4(),
            tz: user_service.repo.tz(),
        };

        let res = user_service.reopen(TaskID::new_v4(), user_context).await;
        assert!(res.is_err());
        if let Err(err) = res {
