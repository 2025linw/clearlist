...
async fn init() -> (UserContext, TaskID, TaskService<MockTaskRepository>) {
    ...
    let task = task_service
        ...
        .await
        .unwrap();
    task_service.delete(task.id, user_context).await.unwrap();
...
mod success {
    ...
    async fn success() {
        ...
        let res = task_service.restore(task_id, user_context).await;
        assert!(res.is_ok());
    }
...
mod existence {
    ...
    async fn not_exists() {
        ...
        let res = task_service.restore(TaskID::new_v4(), user_context).await;
        assert!(res.is_err());
        if let Err(err) = res {
            assert!(matches!(err, Error::NotFound(Resource::Task)))
        }
