...
#[test]
async fn remove_tag_task_existing(pool: PgPool) {
    ...
    let res = repo.remove_tag(test_task.id, user.id, test_tag.id).await;
    assert!(res.is_ok());
    let task = repo
...
#[test]
async fn remove_tag_updates_task_updated_at(pool: PgPool) {
    let user_repo = PgUserRepository::init(pool.clone());
    let repo = PgTaskRepository::init(pool.clone());
    let tag_repo = PgTagRepository::init(pool.clone());
...
#[test]
async fn remove_tag_task_soft_deleted(pool: PgPool) {
    let user_repo = PgUserRepository::init(pool.clone());
    let repo = PgTaskRepository::init(pool.clone());
    let tag_repo = PgTagRepository::init(pool.clone());
...
#[test]
async fn remove_tag_task_deleted(pool: PgPool) {
    let user_repo = PgUserRepository::init(pool.clone());
    let repo = PgTaskRepository::init(pool.clone());
    let tag_repo = PgTagRepository::init(pool.clone());
...
#[test]
async fn remove_tag_task_not_owned(pool: PgPool) {
    let user_repo = PgUserRepository::init(pool.clone());
    let repo = PgTaskRepository::init(pool.clone());
    let tag_repo = PgTagRepository::init(pool.clone());
    ...
    let res = repo.remove_tag(other_task.id, user.id, other_tag.id).await;
    assert!(res.is_err());
    if let Err(err) = res {
        assert!(matches!(
            err,
...
#[test]
async fn remove_tag_task_nonexistent(pool: PgPool) {
    let user_repo = PgUserRepository::init(pool.clone());
    let repo = PgTaskRepository::init(pool.clone());
    let tag_repo = PgTagRepository::init(pool.clone());
    ...
    let res = repo
        .remove_tag(TaskID::new_v4(), user.id, test_tag.id)
        .await;
    assert!(res.is_err());
    if let Err(err) = res {
        assert!(matches!(
...
#[test]
async fn remove_tag_nonexistent(pool: PgPool) {
    let user_repo = PgUserRepository::init(pool.clone());
    let repo = PgTaskRepository::init(pool.clone());
    ...
    let res = repo
        .remove_tag(test_task.id, user.id, TagID::new_v4())
        .await;
    assert!(res.is_ok());
}

#[test]
async fn remove_tag_already_removed(pool: PgPool) {
    let user_repo = PgUserRepository::init(pool.clone());
    let repo = PgTaskRepository::init(pool.clone());
    let tag_repo = PgTagRepository::init(pool.clone());
    ...
    let test_task = repo
        ...
        .await
        .unwrap();
    repo.remove_tag(test_task.id, user.id, test_tag.id)
        .await
        .unwrap();
