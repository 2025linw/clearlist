...
mod success {
    ...
    async fn success() {
        ...
        let res = client.set_tags(true, task_id, vec![tag_id]).await;
...
mod error {
    ...
    async fn not_authenticated() {
        ...
        let res = client.set_tags(false, task_id, vec![tag_id]).await;
    ...
    async fn invalid_path_id() {
        ...
        let res = client
            .set_tags(true, "InvalidPath".to_string(), vec![tag_id])
            .await;
    ...
    #[test]
    async fn invalid_tag_ids() {
        // test deserialization
        let (client, task_id, _) = init(true, true).await;
